...
# second model
def new_core_cnn_layers(img_in, l4_stride=1):

    x = img_in
    x = Convolution2D(filters=48, kernel=5, strides=2, layer_num=1)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)

    x = Convolution2D(filters=64, kernel=5, strides=2, layer_num=2)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)

    x = Convolution2D(filters=128, kernel=5, strides=2, layer_num=3)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)

    x = Convolution2D(filter=128, kernel=3, strides=l4_stride, layer_num=4)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((2, 2))(x)

    x = Convolution2D(filter=64, kernel=3, strides=1, layer_num=5)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((2, 2))(x)
...
def default_categorical(input_shape=(120, 160, 3)):
    drop = 0.2
    img_in = Input(shape=input_shape, name='img_in')
    x = new_core_cnn_layers(img_in, drop, l4_stride=2)
    x = Dense(100, activation='relu', name="dense_1")(x)
    x = Dropout(drop)(x)
    x = Dense(50, activation='relu', name="dense_2")(x)
    x = Dropout(drop)(x)
